...
public class IpV4Util
{
    ...
    // A类保留地址范围：1.0.0.1---126.255.255.254
    private static long[] IpATypeRange;
    // B类保留地址范围：128.0.0.1---191.255.255.254
    private static long[] IpBTypeRange;
    // C类保留地址范围：192.168.0.0～223.255.255.255
    private static long[] IpCTypeRange;

    // A,B,C类地址的默认mask
    private static long DefaultIpAMask;
    private static long DefaultIpBMask;
    private static long DefaultIpCMask;

    // 初始化
    static
    {
        IpATypeRange = new long[2];
        IpATypeRange[0] = getIpV4Value("1.0.0.1");
        IpATypeRange[1] = getIpV4Value("126.255.255.254");

        IpBTypeRange = new long[2];
        IpBTypeRange[0] = getIpV4Value("128.0.0.1");
        IpBTypeRange[1] = getIpV4Value("191.255.255.254");

        IpCTypeRange = new long[2];
     ...
     * 判断一个ip是否在一个给定的网段中
     */
    public static boolean checkInSegment(String ip, String ipStart, String ipEnd) {
        if(IpV4Util.compareIpV4s(ipEnd, ip) && IpV4Util.compareIpV4s(ip, ipStart)) {
            return true;
        } else {
            return false;
        }
     ...
     * @return
     */
    public static boolean checkSameSegment(String ip1,String ip2, long mask)
    {
        // 判断IPV4是否合法
        if(!ipV4Validate(ip1) || !ipV4Validate(ip2)) {
            return false;
        }
        long ipValue1 = getIpV4Value(ip1);
        long ipValue2 = getIpV4Value(ip2);
    ...
    {
        long mask = getDefaultMaskValue(ip1);     // 获取默认的Mask
        return checkSameSegment(ip1,ip2,mask);
    }


    /**
     * 判断ipV4或者mask地址是否合法，通过正则表达式方式进行判断 true合法; false不合法
     * @param ipv4
     */
     ...
     * @return
     */
    public static boolean compareIpV4s(String ip1,String ip2)
    {
        boolean result = true;
        long ipValue1 = getIpV4Value(ip1);     // 获取ip1的32bit值
        long ipValue2 = getIpV4Value(ip2); // 获取ip2的32bit值
    ...
    {
        long inValue = getIpV4Value(ipV4);
     ...
     * @return mask 32bit值
     */
    public static long getDefaultMaskValue(String anyIpV4)
    {
     ...
     * @return
     */
    public static String trans2IpStr(long ipValue)
    {
        // 保证每一位地址都是正整数
    ...
    public static long getIpV4Value(String ipOrMask)
    {
        Long num = 0L;
